<div class="content-wrapper">
    ...
    ?>
    <section class="content" id="content-pengembalian">
        <div class="box">
                    ...
                    <table class="table table-bordered table-striped table-hover" id="dataTable">
                        ...
                        <tbody>
                            ...
                            foreach ($peminjaman->result_array() as $row) {
                                $totalPerangkat = $this->db->query('SELECT id FROM tb_dipinjam WHERE idPeminjaman="' . $row['id'] . '"')->num_rows();
                                $tolakPerangkat = $this->db->query('SELECT id FROM tb_dipinjam WHERE status="Ditolak" AND idPeminjaman="' . $row['id'] . '"')->num_rows();
                                $prosesPerangkat = $this->db->query('SELECT id FROM tb_dipinjam WHERE status="Diproses" AND idPeminjaman="' . $row['id'] . '"')->num_rows();

                                if ($totalPerangkat != $tolakPerangkat && $totalPerangkat != $prosesPerangkat) :
                            ?>
                                    <tr>
                                        <td><?= $no++ ?></td>
                                        <td><?= $row['nomor'] ?></td>
                                        ...
                                        <td>
                                            <?= $this->db->query('SELECT id FROM tb_dipinjam WHERE status="Disetujui" AND idPeminjaman="' . $row['id'] . '" AND idUserpengembalian != 0')->num_rows(); ?> Dikembalikan
                                        </td>
                                        <!-- <td><?= date('d F Y H:i:s', strtotime($row['terdaftar'])) ?></td> -->
                                        <td>
                                            <a href="<?= base_url('admin/pengembalian/proses/') . $row['id'] ?>" class="btn btn-primary btn-xs" style="margin-bottom: 1px">
                                            ...
                                            $tglKembali = strtotime($row['tanggalKembali']);
